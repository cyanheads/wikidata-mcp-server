...
RUN --mount=type=cache,target=/root/.bun/install/cache \
    bun install --production --frozen-lockfile --omit=peer --ignore-scripts
...
RUN mkdir -p /var/log/wikidata-mcp-server && chown -R bun:bun /var/log/wikidata-mcp-server
...
ENV LOGS_DIR="/var/log/wikidata-mcp-server"
